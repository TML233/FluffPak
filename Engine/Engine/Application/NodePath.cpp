...
namespace Engine {
	...
	String NodePath::GetName(int32 index) const {
		ERR_ASSERT_ACTION(index >= 0 && index < GetNameCount(), "index out of bounds.", return String());
	...
	String NodePath::GetSubname(int32 index) const {
		ERR_ASSERT_ACTION(index >= 0 && index < GetSubnameCount(), "index out of bounds.", return String());
